function [contact,WNA,WTA,WNM,WTM,CP] = CollisionCheckLCP(AH_B,AH_Bm,obj,c)
...
%            c                   : struct with contant vluaes such as
%                                  c.dimd          : 1x1 double, discretization of friction cone [-]
%
...
WTM = zeros(6*Nobj,dimd); %Preallocate: each contact point is two columns with 6xNobj rows
WNA = zeros(6*Nobj,1); %Preallocate: each contact point is one column with 6xNobj rows
WTA = zeros(6*Nobj,dimd); %Preallocate: each contact point is two columns with 6xNobj rows
    ...
    if contact(II)        
        ...
        if isat <7
            ...
            for ii = 1:length(p_i) %loop over the contact points
                for id = 1:2 %loop over the two dimensions of the surface
                    if abs(B1p2(id,p_i(ii)))<(obj{ibref}.dim(id)/2) %If the point is inside the dimension of the object
                        ...
                        index = find(vecnorm(B1p2(:,p_i(ii))-B1p1)==min(vecnorm(B1p2(:,p_i(ii))-B1p1)),1); %Find the contact point on reference body 1 closest to body 2
            ...
            for jj = 1:2 %loop over the two bodies
                ...
                if jj ==1
                    ...
                    idx2 = find(dis4==min(dis4),1); %Index of parallel edge closest to other body
        ...
        for icp = 1:length(B1cp(1,:)) %Count over the contact points between these two bodies
            ...
            if obj{bi1}.dynamics
                ...
                WTM((6*(bi1-1)+1:6*(bi1-1)+6),(dimd*(cntr-1)+1:dimd*(cntr-1)+dimd)) = (D* [-AH_Bm(1:3,1:3,bi1) AH_Bm(1:3,1:3,bi1)*hat(B1cp(:,icp))])'; %Assuming this works the same way in the tangential direction

                WNA((6*(bi1-1)+1:6*(bi1-1)+6),cntr) = wB1a(:,3); %For the ibref body we compute the force directions
                WTA((6*(bi1-1)+1:6*(bi1-1)+6),(dimd*(cntr-1)+1:dimd*(cntr-1)+dimd)) = (D* [-AH_B(1:3,1:3,bi1) AH_B(1:3,1:3,bi1)*hat(B1cp(:,icp))])'; %Assuming this works the same way in the tangential direction
            end
            ...
            if obj{bi2}.dynamics
                ...
                WTM((6*(bi2-1)+1:6*(bi2-1)+6),(dimd*(cntr-1)+1:dimd*(cntr-1)+dimd)) = (D* [AH_Bm(1:3,1:3,bi2) -AH_Bm(1:3,1:3,bi2)*hat(B2cp(:,icp))])';
                ...
                WTA((6*(bi2-1)+1:6*(bi2-1)+6),(dimd*(cntr-1)+1:dimd*(cntr-1)+dimd)) = (D* [AH_B(1:3,1:3,bi2) -AH_B(1:3,1:3,bi2)*hat(B2cp(:,icp))])';
            end
